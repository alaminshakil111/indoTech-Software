...
const SolutionsDetails = () => {
  ...
  if (!solution)
    return (
      <h2 className="text-center py-20 text-xl"> Solutions Not Found </h2>
    );
  ...
  return (
    <div className="dark:bg-[#192030]">
      <div className="containerCustom px-4 py-10">
        ...
        <img src={solution.img} className="w-full h-64 object-cover rounded-lg shadow" alt={solution.title} />

        <h2 className="text-[24px] lg:text-[36px] font-bold text-[#0072B5] mt-6">{solution.title}</h2>
                ...
                {/* FAQ Section */}
                  ...
                  {solution.details[activeTab].strengths && (
                    <div className="mb-6">
                      ...
                      <ul className="list-disc ml-5 textColor">
                        {solution.details[activeTab].strengths.items.map((item, i) => (
                          <li key={i}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {solution.details[activeTab].support && (
                    <div className="mb-6">
                      ...
                      <ul className="list-disc ml-5 textColor">
                        {solution.details[activeTab].support.items.map((item, i) => (
                          <li key={i}>{item}</li>
                        ))}
                      </ul>
                    </div>
